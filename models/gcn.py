...
class GNNGuard(GCN):
    ...
    def __init__(self,
        ...
        self.prune_edges = True
        self.mimic_ref_impl = False
        self.dense_conv = dense_conv
        self.pruning_weight = torch.randn(2).to(self.device)#torch.tensor([0.5, 0.5])
    ...
    def _edge_weights(self, A, X):
        ...
        np.savetxt('D:/pycode/node_injection_with_camouflage/score/pubmed_guard_scr.csv', Alpha_np, delimiter=',')
        print(self.pruning_weight)
        del N  # free memory
        return Alpha
...
class GCNJaccard(GCN):
    ...
    def fit(self,
        ...
        if adj_t is not None:
            adj = adj_t
        mod_adj = self.get_modified_adj(ftr, adj)
        super().fit(
            pyg_data, adj_t=mod_adj,
            train_iters=train_iters, patience=patience,
            initialize=initialize, verbose=verbose)
    ...
    def predict(self, x=None, edge_index=None):
        ...
        if x is None and edge_index is None:
            ...
            assert type(x) is torch.Tensor
            x = x.to(self.device)
            adj = self.get_modified_adj(x, edge_index).to(self.device)
            return self.forward(x, adj).detach()

    def get_modified_adj(self, ftr, adj_t):
        mod_adj = self.drop_dissimilar_edges(ftr, adj_t)
        ...
        mod_adj = SparseTensor.from_edge_index(
            ...
            sparse_sizes=(ftr.shape[0], ftr.shape[0]))
        return mod_adj

    def to_scipy(self, edge_index):
        sparse_tensor = edge_index.to_sparse()
        # 提取稀疏矩阵的组成部分
        indices = sparse_tensor.indices().numpy()
        values = sparse_tensor.values().numpy()
        # 转换为 SciPy 的 CSR 稀疏矩阵
        scipy_sparse_matrix = sp.csr_matrix((values, (indices[0], indices[1])), shape=edge_index.shape)
    ...
    def drop_dissimilar_edges(self,features, adj, metric='similarity'):
        ...
        if metric == 'distance':
            ...
            if self.binary_feature:
                removed_cnt, edges_scores = dropedge_jaccard(adj_triu.data, adj_triu.indptr, adj_triu.indices, features,
                                               threshold=self.threshold)
                import csv
                with open(f'D:/pycode/node_injection_with_camouflage/score/pubmed_Jaccard_scr.csv', mode='w', newline='') as f:
                    writer = csv.writer(f)
                    writer.writerow(['source', 'target', 'score'])
                    for edge in edges_scores:
                        writer.writerow(edge)
            else:
                removed_cnt, edges_scores = dropedge_cosine(adj_triu.data, adj_triu.indptr, adj_triu.indices, features,
                                              threshold=self.threshold)
                import csv
                with open(f'D:/pycode/node_injection_with_camouflage/score/pubmed_Jaccard_scr.csv', mode='w', newline='') as f:
                    writer = csv.writer(f)
    ...
    #         # 提取 COO 格式的行、列索引和数据
    #         # print(type(adj))
    #         # row = adj.storage.row().cpu().numpy()  # 行索引
    #         # col = adj.storage.col().cpu().numpy()  # 列索引
    #         # values = adj.storage.value().cpu().numpy()  # 非零值
    #         #
    #         # # 构造 COO 矩阵再转 CSR
    #         # coo_matrix = sp.coo_matrix((values, (row, col)), shape=adj.sparse_sizes())
    #         # csr_matrix = coo_matrix.tocsr()  # 转换为 SciPy CSR
...
def dropedge_jaccard(A, iA, jA, features, threshold):
    ...
    for row in range(len(iA) - 1):
        for i in range(iA[row], iA[row + 1]):
            ...
            edges_scores.append((n1, n2, J))

            if J < threshold:
                A[i] = 0
                # A[n2, n1] = 0
                removed_cnt += 1
    return removed_cnt, edges_scores
