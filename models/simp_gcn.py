...
class SimpGCN(ModelBase):
    ...
    def __init__(self,
        ...
        self.adj_normed1 = _utils.normalize_adj_tensor(self.edge_index.to_dense())
    ...
    def _forward(self, fea, adj, weight=None):
        ...
        identity = _utils.sparse_mx_to_torch_sparse_tensor(
            sp.eye(n)).to(self.device)
        x = (s_i * self.gc1(fea, adj) + (1 - s_i) * self.gc1(fea, adj_knn)) + (gamma) * Dk_i * self.gc1(fea, identity)
        ...
        x = (s_o * self.gc2(x, adj) + (1 - s_o) * self.gc2(x, adj_knn)) + (gamma) * Dk_o * self.gc2(x, identity)
